...
import ChangeLanguageComponent from "../select-language/selectLanguageComponent";
import SelectLayoutComponent from "../select-layout/selectLayout";
  ...
  const runCode = async () => {
    ...
    try {
      const { run: result } = await executeCodeFn(
        codeEditorStore.language?.value,
        code
      );
      ...
      <div className="flex items-center justify-between w-full">
        ...
        <div className="inline-flex items-center justify-start gap-1 lg:gap-4 ">
          ...
          <ButtonUi
            ...
            className={
              "bg-[#2a9134] rounded-sm text-white py-[10px] px-4 shadow-[0px_0px_15px_-10px_#00000077]"
            }
            clickHandler={runCode}
            disabled={
              codeEditorStore.language?.value === "Tanlang..." ? true : false
            }
          />
        </div>
      </div>
